...
from ipywidgets.embed import load_template
...
GEO_DATA_COLUMNS = ['PostalCode', 'Latitude', 'Longitude']

TRANSPOSED_COLUMN_NAMES = ['polished_specialty',
                           'AirConditioning',
                           'AlloyWheels',
                           ...
                           'ShockAbsorber',
                           'UltraSonic',
                           'DrivingSupportSystem', 'SpecificModelId', 'PostalCode', 'DateCrawled'] + ['CarAge',
                                                                                                      'CarMake',
                                                                                                      'CarModel',
                                                                                                      'SpecificHtml',
                                                                                                      'Age',
                                                                                                      ...
                                                                                                      'DeductibleAmount'] + [
                              'Name' + str(i // 2 + 1) if i % 2 == 1 else "Price" + str(i // 2 + 1) for i in
                              range(0, 24)]

# row ids of relevant attributes (suspect to change if template changes)
RELEVANT_IDS_READ_VALUE = ['car_make_year', 'car_maker', 'car_model', 'all_trims_list', 'driver_birth_year',
                           'driver_licence_year', 'bonus_malus', 'Price',
                           'Name'] + \
                          ['Name' + str(i // 2 + 1) if i % 2 == 1 else "Price" + str(i // 2 + 1) for i in range(2, 24)]
RELEVANT_IDS_VALUE = ['car_trim', 'postal_code']
RELEVANT_IDS_USE = ['polished_specialty',
                    'AirConditioning',
                    'AlloyWheels',
                    'AutomaticTransmission',
                    'LeatherSeats',
...
#geoData = pd.read_table(GEO_DATA_PATH, header=None, usecols=[1, 9, 10])
#geoData.columns = GEO_DATA_COLUMNS
#geoData = geoData.drop_duplicates(subset=['PostalCode'])
...
#
#
# # function for extracting kW, ccm and kg data from the Html
#
#
#
#
# name_cols = [col for col in transposed.columns if 'Name' in col]
#
#
# for name_col in name_cols:
#     transposed[name_col] = transposed[name_col].apply(lambda x: 'Wáberer' if x == 'Gránit' else x)
#
...
#transposed = transposed.drop(['Name' + str(i) for i in range(1, 13)] + ['Price' + str(i) for i in range(1, 13)], axis=1)

#final_cols = ['DateCrawled', 'isRecent', 'CarMake', 'CarModel', 'CarAge', 'ccm', 'kw', 'kg', 'car_value',
              # 'CarMakerCategory', 'PostalCode',
              # 'PostalCode2', 'PostalCode3', 'Category', 'Longitude', 'Latitude', 'Age', 'LicenseAge', 'BonusMalus',
              # 'DeductiblePercentage', 'DeductibleAmount',
              # 'BonusMalusCode'] + [col for col in transposed.columns if "_price" in col and 'NAN_price' != col]
#data = transposed[final_cols]
...
def load_crawled_profiles(files : list, transposed_config : dict) -> pd.DataFrame:
    ...
    with ThreadPoolExecutor() as executor:
        futures = {executor.submit(process_file, file, value_names, read_value_names, use_names): file for file in files}
